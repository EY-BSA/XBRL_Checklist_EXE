...
def enrich_axis_domain_check(std: StandardTaxonomy, path: str) -> None:
    ...
    구조: result 시트, Table_Number / Name / Axis_Domain(축|도메인|멤버) 컬럼 사용.
    """
    import pandas as pd
    df = pd.read_excel(path, sheet_name='result',
                       usecols=['Table_Number', 'Name', 'Axis_Domain'])
    df['Table_Number'] = df['Table_Number'].astype(str).str.strip()
    df['Name']         = df['Name'].astype(str).str.strip()

    # 3-1: Axis/Domain/Member 정합성 검토용 (Axis_Domain not null 행만)
    ...
    # 4-x: 전용요소 검토용 — Alteryx Find-Replace R 인풋과 동일하게
    #       TABLE_NUMBER IN (해당 코드) 필터 후 Name 집합 추출
...
def enrich_dart_negate_check(std: StandardTaxonomy, path: str) -> None:
    ...
    구조: result 시트, Name / Negate(negate|-) 컬럼 사용.
    dart_taxonomy.xlsm 없이도 7-1 체크 가능하도록 독립적으로 로드.
    """
    import pandas as pd
    df = pd.read_excel(path, sheet_name='result', usecols=['Name', 'Negate'])
