...
def _c1_2(rows, data):
    ...
    Alteryx 로직 (Node 418→415→563):
      ...
      GroupBy table_label_ko → Sum flags
    ...
    grp = df.groupby('table_label_ko')[['flag_1', 'flag_2', 'flag_3']].sum().reset_index()
    grp.columns = ['table_label_ko', 'sum_f1', 'sum_f2', 'sum_f3']
    ...
    joined = left.merge(df, on='table_label_ko', how='inner')
...
def _c3_2(rows, data):
    """3-2: 공시금액의 사용 적정성 검토
    Alteryx 로직:
      Left:  축_도메인 == "축" → TABLE_NUMBER + table_label_ko groupby → count >= 2
      Right: 구분 not in ("FOOTNOTES", "TABLE")
      Inner Join → Name == "ReportedAmountMember" → 검토대상
    """
    r = CheckResult('Checklist_3-2')

    df = pd.DataFrame(rows)

    axis_df = df[df['축_도메인'] == '축']
    group_counts = (axis_df
                    .groupby(['role_uri', 'TABLE_NUMBER', 'table_label_ko'])
                    .size()
                    .reset_index(name='axis_count'))
    left = group_counts[group_counts['axis_count'] >= 2][['role_uri', 'TABLE_NUMBER', 'table_label_ko']]

    right = df[~df['구분'].isin(['FOOTNOTES', 'TABLE'])]

    joined = left.merge(right, on=['role_uri', 'TABLE_NUMBER', 'table_label_ko'], how='inner')
...
def _c7_1(rows, data):
    ...
    # ── 6. 검토대상만 필터 → CheckIssue 변환 ────────────────────────────────
    filtered = Negate_Check[
        (Negate_Check['Client_Negate'] == 'negate') &
        (Negate_Check['검토여부'] == '검토대상')
    ]
    for _, row in filtered.iterrows():
        r.issues.append(_mk(row.to_dict(), 'Client_Negate ≠ DART_Negate — Negate 적용 여부 검토 필요', data))
...
def _c7_2(rows, data):
    ...
    Alteryx 로직 (Node 423→428→424→429):
      ...
      Filter(424): Name에 CashFlowsFromUsedInOperatingActivitiesLineItems 포함 라인만
      Filter(429): NextValue가 ProfitLoss가 아닌 라인 → 검토대상
    """
    r = CheckResult('Checklist_7-2')
